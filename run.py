...
def camera():
    ...
    for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True): # Capture pi camera frames
        ...
        if cnt is not None: # is there something there?
            ...
            cv2.putText(img,label,(50,150), font,8,(66,36,251),2)  # displaying the predicted letter on the main screen
            cv2.putText(img,text,(50,450), font,3,(66,36,251),2)
...
def camera2():
    ...
    for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True): # Capture pi camera frames
        ...
        cv2.rectangle(crop_img, (x, y), (x+w, y+h), (66, 36, 251), 0)
        ...
        if count_defects == 1:
            cv2.putText(img,"One", (50, 50), cv2.FONT_HERSHEY_DUPLEX, 2, 2)
        elif count_defects == 2:
            str = "Two"
            cv2.putText(img, str, (5, 50), cv2.FONT_HERSHEY_DUPLEX, 1, 2)
